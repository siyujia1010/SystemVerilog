...
对照学习记录，这一组断言对应的是 req/ack 协议的几条文字规格（大致方向，复习时建议自己重新对一遍教材原文重新推导严谨表述）：

- **(1)**：没有 `req` 的时候不该有 `ack`（"未经请求不得响应"，防止虚假 ack）。
- **(2)**：核心的 req→ack 超时检查——用 `(!req || ack) ##1 req` 这个"干净的 req 上升沿"（前一拍没有正在进行的握手）作为前提，命中后允许 `req && !ack` 最多持续 100 拍，之后 `ack` 必须出现。这里 `(!req || ack) ##1 req` 就是笔记里提到的**"影子寄存器"技巧**的简化版：用一个组合条件去捕捉"这是一次全新的握手开始"，避免把同一次握手的中间状态误判成新的 req。
- **(3)**：`ack` 一旦出现（`$rose(ack)`），最多保持 100 拍后必须回落（`ack` 不能一直悬空拉高）。
- **(4)**：`ack` 到达且 `req` 仍为高的那一拍，下一拍 `req` 必须撤销——即握手完成后 `req` 要及时收回，不能赖着不走。
- **(5)**：健康性检查，`req` 不能出现未知态 `X`。
- **(6)**：这是一条数值稳定性/回绕（wrap-around）检查，用 `$stable` + `$past` 判断某个计数值 `value` 与上一拍的差是否符合预期（含跨越最大值时的回绕情况），常用于给带 ID/计数域的握手信号做数据完整性校验，为下面的"ID 匹配进阶版"做铺垫。

> 复习建议：拿这 6 条去对照"7 条文字规格"重新逐条复述一遍中文规格，直到能不看代码就把每条断言口头描述出来。
