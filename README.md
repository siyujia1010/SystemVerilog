...
| [03-data-types/05-streaming-operator.md](03-data-types/05-streaming-operator.md) | 流操作符深入：`{<<n{...}}` 切片粒度、nibble swap / byte swap（04-arrays §5 的深入版） |

### 第四章：进程与同步（task 生命周期 / 事件 / 信号量）

> 与第二章 [fork / join 与进程控制](02-assertions/04-fork-and-process-control.md) 是外层/内层关系：fork/join 管"线程怎么启动和收拢"，本章管"线程之间共享什么、怎么同步"。

| 文件 | 内容 |
|---|---|
| [04-process-sync/01-tasks-process-sync.md](04-process-sync/01-tasks-process-sync.md) | `automatic` vs `static` 并发竞态、`$display/$strobe/$monitor`、`@` vs `wait(.triggered)` 竞态、semaphore |

### 第五章：随机约束（Constrained Random）

| 文件 | 内容 |
|---|---|
| [05-constrained-random/01-randomize-and-constraints.md](05-constrained-random/01-randomize-and-constraints.md) | `$urandom_range`、`randomize() with` 与 `local::`、`rand_mode` / `constraint_mode`、soft、`dist`（`:=` vs `:/`）、数组归约约束、`solve...before` 概率偏置 |
...
- [x] 第三章：流操作符深入（nibble swap / byte swap）—— 已完成
- [x] 第四章：进程与同步（automatic/static 竞态、@ vs wait、semaphore）—— 已完成
- [x] 第五章：随机约束（inline 约束名字解析、soft、dist、solve...before）—— 已完成
...
- 路科验证 V0 课程 —— 第 2 讲《任务和函数》、第 3 讲《随机约束》《进程间同步和通信》（第四、五章笔记末尾附有与课程的对应关系表）
- 个人手写笔记 (dv笔记)
